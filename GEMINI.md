...
1. **사용자 인증:** 간단한 아이디 & 비밀번호 기반 회원 가입 및 JWT 로그인
...
## 4. Gemini Code Assist 컨텍스트 활용 지침 (Context Awareness)

AI 어시스턴트는 코드를 생성하거나 수정하기 전, 다음의 로컬 컨텍스트를 반드시 최우선으로 참고합니다.

- **데이터베이스 스키마 동기화:** JPA 엔티티(Entity) 클래스나 SQL 쿼리를 작성할 때는 반드시 프로젝트 내의 `database/schema.sql` 파일을 먼저 스캔하여 테이블 구조, 컬럼명, 제약조건을 정확히 반영할 것. 임의로 컬럼을 추측하여 생성하지 않습니다.
- **프로젝트 아키텍처 참조:** 프론트엔드/백엔드 코드를 작성하기 전, 현재 워크스페이스에 열려있는 디렉토리 구조(Feature-Driven)와 기존 컴포넌트(`src/features/...`)의 작성 패턴을 분석하여 일관된 스타일을 유지할 것.
- **의존성 파악:** 새로운 라이브러리 함수나 훅을 제안할 때는 `package.json` 또는 `build.gradle.kts`에 명시된 버전(Spring Boot 3.x, React 18 등)과 호환되는 최신 문법(Idiomatic Code)을 사용할 것.